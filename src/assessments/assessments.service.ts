...
import {
  ...
  StartAttemptDto, SubmitAttemptDto, AutoSaveDto, ReviewAnswerDto,
  QuestionType, AttemptStatus, FeedbackMode,
...
export class AssessmentsService {
  ...
  async remove(id: number) {
    ...
    if (a._count.attempts > 0 && a.status === 'PUBLISHED') {
      throw new ForbiddenException('Avaliação publicada com tentativas não pode ser eliminada. Archive-a primeiro.');
    }
    ...
    if (assessment.cooldownHours > 0) {
      ...
      if (lastAttempt) {
        ...
        if (timeSinceLast < cooldownMs) {
          ...
          throw new ForbiddenException(`Cooldown activo. Nova tentativa disponível em ${hoursLeft}h`);
        }
      }
    }
  ...
  async submitAttempt(userId: number, dto: SubmitAttemptDto) {
    ...
    for (const q of questions) {
      ...
      if (q.type === 'MULTIPLE_CHOICE_SINGLE' || q.type === 'TRUE_FALSE') {
        ...
        const opts          = q.options ? JSON.parse(q.options) : [];
        const correctIndices= opts.map((o: any, i: number) => o.isCorrect ? i : -1).filter((i: number) => i >= 0);
      ...
      results.push({
        ...
        options:       assessment.feedbackMode !== 'RESULT_ONLY' ? (q.options ? JSON.parse(q.options) : null) : undefined,
      });
    }
    ...
    const score = autoGradableWeight > 0
      ? Math.round((earnedWeight / autoGradableWeight) * 100)
      : 0;
  ...
  async getAttemptDetail(attemptId: number, userId: number) {
    const attempt = await this.prisma.assessmentAttempt.findFirst({
      ...
      include: {
        assessment: { select: { id: true, title: true, passingScore: true, allowReview: true, feedbackMode: true } },
        answers: {
  ...
  async reviewAnswer(dto: ReviewAnswerDto, reviewerId: number) {
    ...
    if (pendingReview === 0) {
      ...
      const totalEarned = allAnswers.reduce((s, a) => {
        const pts = (a as any).manualScore !== null
      ...
      const passed = score >= (attempt.assessment as any).passingScore;
  ...
  async getAnalytics(assessmentId: number) {
    ...
    const [totalAttempts, passed, failed, avgScore, questions] = await Promise.all([
      this.prisma.assessmentAttempt.count({ where: { assessmentId, status: { not: 'IN_PROGRESS' } } }),
    ...
    const questionStats = await Promise.all(
      questions.map(async q => {
        const [total, correct] = await Promise.all([
          ...
          this.prisma.assessmentAttemptAnswer.count({ where: { questionId: q.id, isCorrect: true } }),
        ]);
    ...
    return {
      assessmentId,
      attempts: { total: totalAttempts, passed, failed, inProgress: totalAttempts - passed - failed },
