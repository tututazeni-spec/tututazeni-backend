...
export class ExecutiveReportsController {
  ...
  stats() { return this.svc.getReportStats(); }
  ...
  submit(@Param('id', ParseIntPipe) id: number) { return this.svc.submitForReview(id); }
  ...
  @HttpCode(HttpStatus.OK)
  publish(@Param('id', ParseIntPipe) id: number) { return this.svc.publishReport(id); }

  @Patch(':id/archive')
  @ApiOperation({ summary: 'Arquivar relatório' })
  @HttpCode(HttpStatus.OK)
  archive(@Param('id', ParseIntPipe) id: number) { return this.svc.archiveReport(id); }
