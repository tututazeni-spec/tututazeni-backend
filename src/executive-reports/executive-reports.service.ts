...
import {
  CreateExecutiveReportDto, UpdateExecutiveReportDto, ReportFilterDto,
  ApproveReportDto, ReportType, ReportStatus, KpiStatus,
...
export class ExecutiveReportsService {
  ...
  async submitForReview(id: number) {
    ...
    if (r.status !== 'DRAFT') throw new BadRequestException('Apenas relatórios DRAFT podem ser submetidos');
  ...
  async publishReport(id: number) {
    const r = await this.findOne(id) as any;
    if (r.status !== 'APPROVED') throw new BadRequestException('Apenas relatórios aprovados podem ser publicados');
    return this.prisma.executiveReport.update({
      where: { id },
  ...
  async remove(id: number) {
    const r = await this.findOne(id) as any;
    if (r.status === 'PUBLISHED') throw new ForbiddenException('Relatório publicado não pode ser eliminado');
  ...
  async generateAutoReport(
    ...
    const [
      totalUsers, hired, terminated,
      totalEnrollments, completed, completedMonth,
      avgPerf, activePDIs, completedPDIs,
      totalCerts, totalXp, overdueActions,
    ] = await Promise.all([
      ...
      this.prisma.enrollment.count({ where: { user: userWhere, status: 'COMPLETED', completedAt: { gte: monthAgo } } }),
      ...
      this.prisma.developmentPlan.count({ where: { status: 'COMPLETED', user: userWhere, completedAt: { gte: monthAgo } } }),
      this.prisma.certificate.count({ where: { issuedAt: { gte: monthAgo } } }),
      this.prisma.userPoints.aggregate({ _sum: { points: true } }),
      this.prisma.developmentPlanAction.count({ where: { plan: { user: userWhere, status: 'ACTIVE' }, status: { not: 'COMPLETED' }, dueDate: { lt: now } } }),
    ]);
    ...
    const metrics = [
      ...
      { label: 'Saídas no período',     value: terminated,       unit: 'pessoas', status: terminated > 5 ? KpiStatus.RED : KpiStatus.GREEN, target: 3 },
      { label: 'Taxa de turnover',      value: turnoverRate,     unit: '%',       status: turnoverRate > 10 ? KpiStatus.RED : turnoverRate > 5 ? KpiStatus.YELLOW : KpiStatus.GREEN, target: 5 },
      { label: 'Conclusão de cursos',   value: completionRate,   unit: '%',       status: completionRate < 30 ? KpiStatus.RED : completionRate < 60 ? KpiStatus.YELLOW : KpiStatus.GREEN, target: 70 },
      { label: 'Conclusões este mês',   value: completedMonth,   unit: '',        status: KpiStatus.GREEN },
      { label: 'Score médio perf.',     value: avgScore,         unit: '/5',      status: avgScore < 2.5 ? KpiStatus.RED : avgScore < 3.5 ? KpiStatus.YELLOW : KpiStatus.GREEN, target: 4 },
      { label: 'Adopção de PDI',        value: pdiAdoption,      unit: '%',       status: pdiAdoption < 30 ? KpiStatus.RED : pdiAdoption < 60 ? KpiStatus.YELLOW : KpiStatus.GREEN, target: 80 },
      ...
      { label: 'Acções PDI atrasadas',  value: overdueActions,   unit: '',        status: overdueActions > 20 ? KpiStatus.RED : overdueActions > 5 ? KpiStatus.YELLOW : KpiStatus.GREEN, target: 0 },
      { label: 'XP total da plataforma',value: totalXp._sum.points ?? 0, unit: 'xp', status: KpiStatus.GREEN },
    ];
    ...
    if (overdueActions > 10) risks.push(`${overdueActions} acções de PDI atrasadas — risco de abandono`);
    ...
    if (completedPDIs > 0)   achievements.push(`${completedPDIs} Planos de Desenvolvimento concluídos`);
    if (totalCerts > 0)      achievements.push(`${totalCerts} certificados emitidos`);

    const recommendations: string[] = [];
    if (completionRate < 50) recommendations.push('Lançar campanha de reactivação para matrículas inactivas');
    if (pdiAdoption < 60)    recommendations.push('Tornar PDI obrigatório para todos os colaboradores activos');
    if (overdueActions > 0)  recommendations.push('Enviar lembretes automáticos para gestores com acções atrasadas');
  ...
  async getTemplates() {
    return [
      {
        id: 'flash',    name: 'Flash Report (Semanal)',      type: ReportType.FLASH,
        description:    'KPIs rápidos de monitorização semanal',
        sections:       ['headcount', 'learning_week', 'alerts'],
      },
      {
        id: 'monthly',  name: 'Status Report (Mensal)',      type: ReportType.MONTHLY,
        description:    'Evolução mensal vs metas',
        sections:       ['people', 'learning', 'pdi', 'performance'],
      },
      {
        id: 'quarterly',name: 'Executive Summary (Trimestral)', type: ReportType.QUARTERLY,
        description:    'Relatório estratégico completo para C-Level',
        sections:       ['executive_summary', 'people', 'learning', 'pdi', 'performance', 'roi', 'succession'],
      },
      {
        id: 'annual',   name: 'Annual People Report',        type: ReportType.ANNUAL,
        description:    'Consolidado anual com ROI e tendências',
        sections:       ['executive_summary', 'people', 'learning', 'roi', 'succession', 'diversity', 'benchmark'],
      },
    ];
  }
