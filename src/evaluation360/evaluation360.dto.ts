...
import {
  IsString, IsOptional, IsEnum, IsInt, IsBoolean, IsArray,
  IsNumber, IsDateString, IsNotEmpty, Min, Max, MaxLength,
...
// ─── ENUMS ───────────────────────────────────────────────────
export enum CompetencyType { HARD_SKILL = 'HARD_SKILL', SOFT_SKILL = 'SOFT_SKILL', CULTURE = 'CULTURE', LEADERSHIP = 'LEADERSHIP', VITALITY = 'VITALITY', CUSTOM = 'CUSTOM' }
export enum EvaluationModel { DEG_90 = 'DEG_90', DEG_180 = 'DEG_180', DEG_270 = 'DEG_270', DEG_360 = 'DEG_360', HYBRID = 'HYBRID' }
export enum CycleType { TRIMESTRAL = 'TRIMESTRAL', SEMESTRAL = 'SEMESTRAL', ANUAL = 'ANUAL', PROJECT = 'PROJECT', CUSTOM = 'CUSTOM' }
export enum CycleStatus { DRAFT = 'DRAFT', PUBLISHED = 'PUBLISHED', IN_PROGRESS = 'IN_PROGRESS', PROCESSING = 'PROCESSING', COMPLETED = 'COMPLETED', CANCELLED = 'CANCELLED' }
export enum AnonymityMode { ANONYMOUS = 'ANONYMOUS', SEMI_ANONYMOUS = 'SEMI_ANONYMOUS', OPEN = 'OPEN' }
export enum EvaluatorRole { SELF = 'SELF', MANAGER = 'MANAGER', PEER = 'PEER', SUBORDINATE = 'SUBORDINATE', EXTERNAL = 'EXTERNAL' }
export enum QuestionType { LIKERT = 'LIKERT', FREQUENCY = 'FREQUENCY', MULTIPLE_CHOICE = 'MULTIPLE_CHOICE', YES_NO = 'YES_NO', OPEN_TEXT = 'OPEN_TEXT', SITUATIONAL = 'SITUATIONAL' }
export enum FeedbackType { RECOGNITION = 'RECOGNITION', DEVELOPMENT = 'DEVELOPMENT', CHECK_IN = 'CHECK_IN', PULSE = 'PULSE' }
...
export class CreateCompetencyDto {
  ...
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => CompetencyIndicatorDto)
...
export class CreateEvaluationCycleDto {
  ...
  @ApiPropertyOptional({ enum: AnonymityMode }) @IsOptional() @IsEnum(AnonymityMode) anonymityMode?: AnonymityMode;
  ...
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => CycleCompetencyDto)
...
export class PublishCycleDto {
  ...
  @IsOptional() @IsBoolean() sendInvitesNow?: boolean;
}
...
export class BulkAssignEvaluatorsDto {
  ...
  @IsArray() @ValidateNested({ each: true }) @Type(() => AssignEvaluatorDto)
...
export class GenerateReportDto {
  ...
  @IsOptional() @IsString() scope?: 'INDIVIDUAL' | 'TEAM' | 'ORGANIZATIONAL';
...
export class SendRemindersDto {
  ...
  @IsOptional() @IsArray() @IsString({ each: true }) assignmentIds?: string[];
