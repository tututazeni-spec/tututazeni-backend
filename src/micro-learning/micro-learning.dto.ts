...
import {
  ...
  IsBoolean, IsNumber, IsDateString, Min, Max, MaxLength, ValidateNested,
...
export class MicroQuizQuestionDto {
  ...
  @IsArray() @ValidateNested({ each: true }) @Type(() => MicroQuizOptionDto)
...
export class CreateMicroLearningDto {
  ...
  @ApiProperty({ enum: ContentType }) @IsEnum(ContentType)
  contentType!: ContentType;

  @ApiProperty({ enum: ContentLevel }) @IsEnum(ContentLevel)
  level!: ContentLevel;
  ...
  @IsOptional() @IsArray() @ValidateNested({ each: true }) @Type(() => MicroQuizQuestionDto)
...
export class InteractDto {
  ...
  @ApiProperty({ enum: ['LIKE', 'SAVE', 'SKIP'] }) @IsString()
...
export class MicroLearningFilterDto {
  ...
  @ApiPropertyOptional({ enum: ContentType }) @IsOptional() @IsEnum(ContentType)
  contentType?: ContentType;

  @ApiPropertyOptional({ enum: ContentLevel }) @IsOptional() @IsEnum(ContentLevel)
  level?: ContentLevel;

  @ApiPropertyOptional({ enum: ContentStatus }) @IsOptional() @IsEnum(ContentStatus)
  status?: ContentStatus;
