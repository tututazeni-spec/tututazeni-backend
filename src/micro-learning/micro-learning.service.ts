...
export class MicroLearningService {
  ...
  async findAll(filters: MicroLearningFilterDto) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.microLearning.findMany({
        ...
        include: { _count: { select: { dispatch: true } } },
  ...
  async dispatch(dto: DispatchMicroLearningDto) {
    ...
    const dispatches = await Promise.allSettled(
      dto.userIds.map(userId =>
        this.prisma.microLearningDispatch.upsert({
          where: { id: 0 }, // forçar create
          create: { microLearningId: dto.microLearningId, userId, viewed: false },
          update: {},
        }).catch(() =>
          this.prisma.microLearningDispatch.create({
            data: { microLearningId: dto.microLearningId, userId, viewed: false },
          }),
        ),
      ),
    );
  ...
  async getMyFeed(userId: number) {
    return this.prisma.microLearningDispatch.findMany({
      where: { userId },
  ...
  async getDispatchStats(microLearningId: number) {
    ...
    const [total, viewed] = await Promise.all([
      this.prisma.microLearningDispatch.count({ where: { microLearningId } }),
      this.prisma.microLearningDispatch.count({ where: { microLearningId, viewed: true } }),
    ]);
    return {
      microLearningId, total, viewed,
