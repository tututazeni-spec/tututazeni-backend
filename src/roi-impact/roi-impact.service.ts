...
function dateRange(filter: RoiFilterDto): { gte: Date; lte: Date } {
  ...
  const gte = filter.from
    ? new Date(filter.from)
    : new Date(lte.getFullYear() - 1, lte.getMonth(), 1);
  return { gte, lte };
}
...
function buildNarrative(roi: number, benefit: number, cost: number, completions: number): string {
  ...
  const label =
    roi >= 200 ? 'excepcional' : roi >= 100 ? 'muito positivo' : roi >= 0 ? 'positivo' : 'negativo';
  return (
    `O programa de aprendizagem gerou um retorno ${label} de ${roi}% sobre o investimento. ` +
...
export class RoiImpactService {
  ...
  async calculateRoiFull(filter: RoiFilterDto, params: Partial<CalculateRoiDto>) {
    ...
    const [
      ...
      (this.prisma as any).lessonProgress
        .count({
    ...
    return {
      ...
      impact: {
        ...
        perfLift,
        retentionSaved,
        avgCompetency: competencyEvolution._avg.currentLevel
          ? +competencyEvolution._avg.currentLevel.toFixed(2)
          : null,
      },
      confidence,
      assumptions: {
  ...
  async getImpactMetrics(filter: RoiFilterDto = {}) {
    ...
    const retentionRate =
      totalActive > 0 ? pct(totalActive - inactiveThisPeriod, totalActive) : null;
    const engScore = avgSurveyScore._avg.score
      ? +(avgSurveyScore._avg.score * 20).toFixed(1)
      : null; // scale to 100
    ...
    return {
      ...
      levels: {
        ...
        L2_learning: {
          label: 'Aprendizagem',
          completions,
          avgScore: avgAssessmentScore._avg.score
            ? +avgAssessmentScore._avg.score.toFixed(1)
            : null,
        ...
        L3_behaviour: {
          ...
          avgCompetencyLevel: skillAvg._avg.currentLevel
            ? +skillAvg._avg.currentLevel.toFixed(2)
            : null,
          confidence: RoiConfidence.MEDIUM,
        },
        L4_results: {
      ...
      summary: {
        ...
        highPotentials: 0,
        newHiresThisYear: await this.prisma.read.user.count({
  ...
  async getRetentionImpact(filter: RoiFilterDto = {}) {
    ...
    const [total, active, leftInPeriod, prevLeft] = await Promise.all([
      ...
      this.prisma.read.user.count({ where: { ...uWhere, active: true } }),
      this.prisma.read.user.count({
  ...
  async getLearningImpact(filter: RoiFilterDto = {}) {
    ...
    return {
      ...
      quality: {
        ...
        avgCompetencyLevel: avgCompetencyBefore._avg.currentLevel
          ? +avgCompetencyBefore._avg.currentLevel.toFixed(2)
          : null,
        targetCompetencyLevel: avgCompetencyAfter._avg.targetLevel
          ? +avgCompetencyAfter._avg.targetLevel.toFixed(2)
          : null,
      },
      financial: {
        costEstimated,
        benefitEstimated: benefitEst,
  ...
  async simulateWhatIf(dto: WhatIfDto) {
    ...
    return {
      ...
      projected: {
        ...
        completionRate: dto.targetCompletionRate,
        cost: projectedCost,
  ...
  async getExecutiveDashboard(filter: RoiFilterDto = {}) {
    ...
    if (retention.turnoverRate > 15)
      alerts.push({ severity: 'HIGH', message: `Turnover elevado: ${retention.turnoverRate}%` });
    if (performance.lift !== null && performance.lift < 0)
      alerts.push({ severity: 'MEDIUM', message: 'Performance organizacional a deteriorar' });

    return {
      period:
        filter.from && filter.to ? { from: filter.from, to: filter.to } : { label: 'Último ano' },
      headline: {
        overallRoi,
        totalBenefit,
        totalCost,
      ...
      domains: {
        learning: {
          roi: roi.financial.roi,
          completions: roi.volume.completed,
          cost: roi.financial.totalCost,
        },
  ...
  async getProgramLibrary(filter: RoiFilterDto = {}) {
    ...
    return {
      ...
      avgRoi: programs.length
        ? +(programs.reduce((s, p) => s + p.roi, 0) / programs.length).toFixed(1)
        : 0,
  ...
  private buildLearningInsights(completionRate: number, mandatoryRate: number): string[] {
    ...
    if (mandatoryRate < 80)
      out.push(`⚠️ Conformidade de formações obrigatórias abaixo de 80%: ${mandatoryRate}%`);
    return out;
  }
}
