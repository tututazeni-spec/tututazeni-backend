import {
  ...
  IsArray, IsNumber, Min, MaxLength, IsDateString,
...
export class CreateOrgDepartmentDto {
  ...
  @ApiProperty({ example: 'RH' }) @IsString() @MaxLength(20)
  code!: string;
  ...
  @IsOptional() @IsEnum(DepartmentStatus)
...
export class CreateOrgPositionDto {
  ...
  @ApiProperty({ enum: PositionLevel }) @IsEnum(PositionLevel)
  level!: PositionLevel;
...
export class CreateOrgUnitDto {
  @ApiProperty({ example: 'Sede Luanda' }) @IsString() @MaxLength(120)
  name!: string;

  @ApiProperty({ example: 'SEDE-LDA' }) @IsString() @MaxLength(20)
  code!: string;

  @ApiProperty({ enum: UnitType }) @IsEnum(UnitType)
  type!: UnitType;
...
export class RecordOrgChangeDto {
  ...
  @ApiProperty({ enum: OrgChangeType }) @IsEnum(OrgChangeType)
  ...
  @ApiProperty({ description: 'Data efectiva da mudança' }) @IsDateString()
...
export class PositionFilterDto {
  ...
  @ApiPropertyOptional({ enum: PositionLevel }) @IsOptional() @IsEnum(PositionLevel)
  level?: PositionLevel;
