...
export class ReportsService {
  ...
  async competencyGapReport(departmentId?: number) {
    ...
    // ← corrigido: level não existe em UserCompetencySelect; usar currentLevel
    // competency e user adicionados via include separado
    ...
    for (const g of userCompetencies) {
      const cname = g.competency.name; // ← corrigido: acessível via select explícito
      if (!byCompetency[cname]) {
        byCompetency[cname] = { name: cname, count: 0, totalGap: 0, usersWithGap: 0 };
      }
      byCompetency[cname].count++;
      const gap = TARGET_LEVEL - g.currentLevel; // ← corrigido: level → currentLevel
      if (gap > 0) {
