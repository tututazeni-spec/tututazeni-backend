...
import {
  ReportFilterDto, SaveReportDto, CreateScheduleDto,
  ReportCategory, ReportFormat,
} from './reports.dto';
...
function dateRange(filter: ReportFilterDto): { gte: Date; lte: Date } {
  ...
  const from = filter.from ? new Date(filter.from) : new Date(to.getFullYear(), to.getMonth() - 1, 1);
...
export class ReportsService {
  ...
  async headcountReport(filter: ReportFilterDto) {
    ...
    const [total, active, newHires, prevHires] = await Promise.all([
      ...
      this.prisma.user.count({ where: { ...baseWhere, active: true } }),
      this.prisma.user.count({ where: { ...baseWhere, createdAt: { gte: range.gte, lte: range.lte } } }),
    ...
    const byPosition = await (this.prisma as any).position.findMany({
    ...
    return {
      ...
      summary: {
        ...
        newHires, newHiresTrend: trend(newHires, prevHires),
        turnoverRate: total > 0 ? pct(total - active, total) : 0,
      },
      byDepartment: byDept.map(d => ({ id: d.id, name: d.name, count: d._count.users })),
      byPosition:   byPosition.map(p => ({ id: p.id, name: p.name, level: p.level, count: p._count.users })),
  ...
  async turnoverReport(filter: ReportFilterDto) {
    ...
    const [total, inactive, newInPeriod, leftInPeriod] = await Promise.all([
      ...
      this.prisma.user.count({ where: { ...uWhere, active: false } }),
      this.prisma.user.count({ where: { ...uWhere, createdAt: { gte: range.gte, lte: range.lte } } }),
  ...
  async trainingReportFull(filter: ReportFilterDto) {
    ...
    const [
      ...
      this.prisma.enrollment.groupBy({
        ...
        const compls = await this.prisma.enrollment.groupBy({ by: ['courseId'], where: { ...where, status: 'CONCLUIDO' }, _count: { id: true } });
        const compMap= new Map(compls.map(c => [c.courseId, c._count.id]));
        return rows.map(r => ({
      ...
      this.prisma.enrollment.groupBy({
        ...
        const depts = await this.prisma.department.findMany({ where: { id: { in: Object.keys(deptCount).map(Number) } }, select: { id: true, name: true } });
        return depts.map(d => ({ department: d.name, completions: deptCount[d.id] ?? 0 })).sort((a, b) => b.completions - a.completions);
      }),
      this.prisma.performanceReview.aggregate({
        where: { ...(uWhere ? { user: uWhere } : {}), createdAt: { gte: range.gte, lte: range.lte } },
        _avg:  { score: true },
    ...
    return {
      report:  'TRAINING',
      period:  { from: range.gte, to: range.lte },
      summary: { enrollments, completed, inProgress, cancelled, completionRate, abandonment, uniqueLearners: 0 },
  ...
  async skillGapReport(filter: ReportFilterDto) {
    ...
    const records = await (this.prisma as any).userCompetency.findMany({
      where,
      include: {
      competency: { select: { id: true, name: true, type: true } },
      user: { select: { id: true, fullName: true, department: { select: { name: true } }, position: { select: { name: true } } } },
     },
   });
    ...
    for (const r of records) {
      ...
      if (!bySkill[cName]) bySkill[cName] = { competency: r.competency, count: 0, totalGap: 0, usersWithGap: 0, users: [] };
      bySkill[cName].count++;
      if (gap > 0) {
        bySkill[cName].totalGap    += gap;
        bySkill[cName].usersWithGap++;
        bySkill[cName].users.push({ id: r.user.id, fullName: r.user.fullName, dept: r.user.department?.name, level: r.currentLevel, gap });
      }
    }
  ...
  async performanceReportFull(filter: ReportFilterDto) {
    ...
    if (filter.from)     where.createdAt = { gte: new Date(filter.from), ...(filter.to && { lte: new Date(filter.to) }) };
    if (uWhere) where.user = uWhere;
    ...
    const reviews = await this.prisma.performanceReview.findMany({
      ...
      include: {
        user: { select: { id: true, fullName: true, avatarUrl: true,
          department: { select: { name: true } }, position: { select: { name: true, level: true } } } },
      },
    ...
    return {
      ...
      byDepartment: Object.entries(byDept).map(([dept, d]) => ({
        department: dept, count: d.count, avgScore: +(d.sum / d.count).toFixed(2),
  ...
  async engagementReport(filter: ReportFilterDto) {
    ...
    const [
      activeSurveys, totalResponses, surveys,
      recognitions, feedbackCount,
      avgMoodRecent,
    ] = await Promise.all([
      this.prisma.engagementSurvey.count({ where: { status: 'ACTIVE' } }),
      this.prisma.surveyResponse.count({
        where: { createdAt: { gte: range.gte, lte: range.lte }, ...(uWhere ? { user: uWhere } : {}) },
      }),
      this.prisma.engagementSurvey.findMany({
        where:   { status: { in: ['ACTIVE', 'COMPLETED'] } },
        include: { _count: { select: { responses: true } }, responses: { select: { score: true }, take: 200 } },
        orderBy: { createdAt: 'desc' },
        take:    6,
      }),
      (this.prisma as any).recognition?.count({ where: { createdAt: { gte: range.gte, lte: range.lte } } }).catch(() => 0),
      (this.prisma as any).feedback?.count({ where: { createdAt: { gte: range.gte, lte: range.lte } } }).catch(() => 0),
      (this.prisma as any).moodCheckin?.aggregate({ _avg: { mood: true }, where: { createdAt: { gte: range.gte } } }).catch(() => null),
    ]);

    const totalUsers = await this.prisma.user.count({ where: { active: true, ...(uWhere ?? {}) } });
    ...
    const surveyData = surveys.map(s => {
      ...
      const avgScore  = responses.length
        ...
        : null;
      return { id: s.id, title: (s as any).title, type: (s as any).type, responses: s._count.responses, avgScore };
    });

    return {
      report:  'ENGAGEMENT',
      period:  { from: range.gte, to: range.lte },
      summary: { activeSurveys, totalResponses, participationRate, totalUsers, recognitions, feedbackCount },
      avgMood: avgMoodRecent?._avg?.mood ? +(avgMoodRecent._avg.mood).toFixed(1) : null,
      surveys: surveyData,
      insights: participationRate < 50
  ...
  async talentReport(filter: ReportFilterDto) {
    ...
    const [
      totalUsers, hiPos, successionPlans,
      activePlans, completedPlans,
      avgCompetency, avgPerf,
    ] = await Promise.all([
      ...
      this.prisma.developmentPlan.count({ where: { status: 'ACTIVE', isTemplate: false, ...(uWhere ? { user: uWhere } : {}) } }),
      this.prisma.developmentPlan.count({ where: { status: 'COMPLETED', isTemplate: false, ...(uWhere ? { user: uWhere } : {}) } }),
      this.prisma.userCompetency.aggregate({ _avg: { currentLevel: true }, where: uWhere ? { user: uWhere } : {} }),
      this.prisma.performanceReview.aggregate({ _avg: { score: true }, where: uWhere ? { user: uWhere } : {} }),
    ]);
    ...
    return {
      report:  'TALENT',
      summary: {
        totalUsers, hiPos, hiPoRatio: pct(hiPos, totalUsers),
        activePlans, completedPlans, pdpCoverage,
        succession,
        avgCompetency: avgCompetency._avg.currentLevel ? +avgCompetency._avg.currentLevel.toFixed(2) : null,
  ...
  async complianceReport(filter: ReportFilterDto) {
    ...
    const [
      ...
      auditLogs, certifications,
    ] = await Promise.all([
      this.prisma.enrollment.count({
        where: { course: { mandatory: true } as any, ...(uWhere ? { user: uWhere } : {}) },
      }).catch(() => 0),
      this.prisma.enrollment.count({
        where: { course: { mandatory: true } as any, status: 'CONCLUIDO', ...(uWhere ? { user: uWhere } : {}) },
      }).catch(() => 0),
    ...
    return {
      ...
      period:  { from: range.gte, to: range.lte },
      summary: {
        mandatoryTotal, mandatoryCompleted, mandatoryRate,
  ...
  async payrollSummary(period: string) {
    ...
    return {
      ...
      totals: Object.fromEntries(Object.entries(totals).map(([k, v]) => [k, +(v as number).toFixed(2)])),
      generatedAt: new Date(),
    };
  }
  ...
  async competencyGapReport(departmentId?: number) {
    ...
    const records = await this.prisma.userCompetency.findMany({
      where,
      include: { competency: true, user: { select: { id: true, fullName: true, department: true } } },
    });
    ...
    for (const g of records) {
      ...
      if (gap > 0) { byComp[n].totalGap += gap; byComp[n].usersWithGap++; }
    }
  ...
  async platformUsageReport(filter: ReportFilterDto) {
    ...
    const [
      contentViews, topContent,
      avatarSessions, surveySubmissions,
      auditActions, activeUsers,
    ] = await Promise.all([
      this.prisma.auditLog.count({
        where: { action: 'CONTENT_VIEW', entity: 'ContentAsset', timestamp: { gte: range.gte, lte: range.lte } },
      }).catch(() => 0),
      ...
      this.prisma.avatarSession.count({ where: { startedAt: { gte: range.gte, lte: range.lte } } }),
      this.prisma.surveyResponse.count({ where: { createdAt: { gte: range.gte, lte: range.lte } } }),
      this.prisma.auditLog.count({ where: { timestamp: { gte: range.gte, lte: range.lte } } }).catch(() => 0),
      this.prisma.auditLog.groupBy({
        by:    ['userId'],
        where: { timestamp: { gte: range.gte, lte: range.lte } },
        _count:{ id: true },
    ...
    return {
      ...
      topContent: (topContent as any[]).map((v: any) => ({
  ...
  async saveReport(userId: number, dto: SaveReportDto) {
    return (this.prisma as any).savedReport?.create({
      ...
      return { id: null, message: 'Relatório guardado (modelo savedReport ausente — execute migration)', ...dto };
    });
  }
  ...
  async getTemplates() {
    const templates = await (this.prisma as any).savedReport?.findMany({
  ...
  async createSchedule(userId: number, dto: CreateScheduleDto) {
    ...
    }).catch(() => ({ message: 'Agendamento registado (modelo reportSchedule ausente — execute migration)', ...dto }));
  }
  ...
  async deleteSchedule(scheduleId: number) {
    await (this.prisma as any).reportSchedule?.update({ where: { id: scheduleId }, data: { active: false } }).catch(() => null);
  ...
  async getInsights(filter: ReportFilterDto) {
    ...
    if (training.summary.completionRate < 60)
      insights.push({ type: 'LEARNING', severity: 'HIGH',
    ...
    if (training.summary.abandonment > 25)
      insights.push({ type: 'LEARNING', severity: 'MEDIUM',
    ...
    if (atRiskCount > 0)
      insights.push({ type: 'PERFORMANCE', severity: 'HIGH',
    ...
    if ((performance.summary.avgScore ?? 0) < 3)
      insights.push({ type: 'PERFORMANCE', severity: 'MEDIUM',
    ...
    if (talent.summary.pdpCoverage < 40)
      insights.push({ type: 'TALENT', severity: 'HIGH',
    ...
    if (talent.summary.succession < 5)
      insights.push({ type: 'TALENT', severity: 'MEDIUM',
    ...
    if (engagement.summary.participationRate < 50)
      insights.push({ type: 'ENGAGEMENT', severity: 'MEDIUM',
  ...
  private buildTrainingInsights(completionRate: number, abandonment: number): string[] {
    ...
    else if (completionRate >= 60) out.push(`⚠️ Taxa de conclusão aceitável: ${completionRate}% — há margem de melhoria`);
    else out.push(`🚨 Taxa de conclusão baixa: ${completionRate}% — rever conteúdo e metodologia`);
    if (abandonment > 25) out.push(`⚠️ Alta taxa de abandono (${abandonment}%) — considerar microlearning`);
    return out;
  }
  ...
  private buildTalentInsights(hiPos: number, total: number, pdpCoverage: number, succession: number): string[] {
    const out = [];
    const hiPoRatio = pct(hiPos, total);
    if (hiPoRatio >= 15)     out.push(`🌟 Alto índice de High Potentials: ${hiPoRatio}% da força de trabalho`);
  ...
  private getBuiltInTemplates() {
    return [
      { id: 'TPL_HEADCOUNT',   name: 'Headcount Report',       category: 'HR',          reportKey: 'headcount',   description: 'Total de colaboradores por departamento e cargo' },
      { id: 'TPL_TURNOVER',    name: 'Turnover Analysis',      category: 'HR',          reportKey: 'turnover',    description: 'Taxa de saída e retenção por período' },
      { id: 'TPL_TRAINING',    name: 'Relatório de Formação',  category: 'LEARNING',    reportKey: 'training',    description: 'Conclusões, abandono e horas de treino' },
      { id: 'TPL_SKILL_GAP',   name: 'Gaps de Competências',   category: 'LEARNING',    reportKey: 'skill-gap',   description: 'Lacunas de skills por departamento' },
      { id: 'TPL_PERFORMANCE', name: 'Performance Report',     category: 'PERFORMANCE', reportKey: 'performance', description: 'Avaliações, distribuição e top performers' },
      { id: 'TPL_TALENT',      name: 'Talent Intelligence',    category: 'TALENT',      reportKey: 'talent',      description: 'HiPos, PDI, sucessão e competências' },
      { id: 'TPL_ENGAGEMENT',  name: 'Engagement Report',      category: 'ENGAGEMENT',  reportKey: 'engagement',  description: 'Surveys, eNPS e participação' },
      { id: 'TPL_COMPLIANCE',  name: 'Compliance Report',      category: 'COMPLIANCE',  reportKey: 'compliance',  description: 'Formações obrigatórias e certificações' },
      { id: 'TPL_USAGE',       name: 'Platform Usage',         category: 'OPERATIONAL', reportKey: 'usage',       description: 'Conteúdos mais vistos e sessões activas' },
    ];
  }
}
