...
import {
  ...
  IsEnum, IsArray, IsDateString, Min, Max, MaxLength,
...
export class CreateInstructorProfileDto {
  ...
  @IsOptional() @IsEnum(InstructorType)
...
export class CreateCohortDto {
  ...
  @ApiProperty({ description: 'ID do curso associado' }) @IsInt()
  courseId!: number;

  @ApiPropertyOptional({ enum: CohortModalidade, default: CohortModalidade.ONLINE })
  @IsOptional() @IsEnum(CohortModalidade)
  modalidade?: CohortModalidade;

  @ApiPropertyOptional({ default: 30 }) @IsOptional() @IsInt() @Min(1)
...
export class UpdateCohortDto extends PartialType(CreateCohortDto) {
  @ApiPropertyOptional({ enum: CohortStatus }) @IsOptional() @IsEnum(CohortStatus)
  status?: CohortStatus;
}
...
export class InstructorReviewDto {
  ...
  @ApiProperty() @IsString() @MaxLength(1000)
  comment!: string;
}
...
export class InstructorFilterDto {
  ...
  @ApiPropertyOptional({ enum: InstructorType }) @IsOptional() @IsEnum(InstructorType)
  instructorType?: InstructorType;
