...
import {
  GlobalSearchDto, TypedSearchDto, AutocompleteDto,
  SearchEntityType, SearchSortBy,
} from './search.dto';
...
function relevanceScore(item: any, q: string, fields: string[]): number {
  ...
  for (const f of fields) {
    ...
    if (val === lq)         score += 100;  // exact match
    else if (val.startsWith(lq)) score += 60; // prefix match
...
export class SearchService {
  ...
  async globalSearch(q: string, userId: number, dto: Partial<GlobalSearchDto> = {}) {
    ...
    if (types.includes(SearchEntityType.CONTENT))
      fetchers.push(this.searchContent(q, { limit }));
    else fetchers.push(Promise.resolve([]));

    if (types.includes(SearchEntityType.PDI))
      fetchers.push(this.searchPdis(q, { limit, userId }));
    else fetchers.push(Promise.resolve([]));
    ...
    const [users, courses, documents, content, pdis, competencies, scenarios] = await Promise.all(fetchers);
    ...
    return {
      ...
      grouped: {
        users, courses, documents, content, pdis, competencies, scenarios,
      },
    };
  }
  ...
  private async searchUsers(q: string, opts: { limit: number; departmentId?: number }): Promise<any[]> {
    const where: any = {
      OR: [
        { fullName:  iLike(q) },
        { email:     iLike(q) },
    ...
    return users.map(u => normalise(
      SearchEntityType.USER, u.id, u.fullName,
  ...
  private async searchCourses(q: string, opts: { limit: number; category?: string }): Promise<any[]> {
    const where: any = {
    ...
    const courses = await (this.prisma as any).course.findMany({
      where,
      select: { id: true, title: true, category: true, workloadHours: true, mandatory: true, thumbnailUrl: true },
      take:   opts.limit,
    });

    return courses.map(c => normalise(
      SearchEntityType.COURSE, c.id, c.title, c.category ?? '',
  ...
  private async searchDocuments(q: string, opts: { limit: number }): Promise<any[]> {
    const articles = await safeM(this.prisma, 'knowledgeArticle').findMany({
      where: { OR: [{ title: iLike(q) }, { description: iLike(q) }, { tags: iLike(q) }] },
      select: { id: true, title: true, description: true, category: true },
      take:   opts.limit,
    }).catch(() => [] as any[]);

    return (articles as any[]).map((d: any) => normalise(
      SearchEntityType.DOCUMENT, d.id, d.title, d.description ?? d.category ?? '',
      {}, `/knowledge/${d.id}`,
    ));
  }
  ...
  private async searchContent(q: string, opts: { limit: number }): Promise<any[]> {
    ...
    return assets.map(c => normalise(
      SearchEntityType.CONTENT, c.id, c.title, c.type ?? '',
  ...
  private async searchPdis(q: string, opts: { limit: number; userId: number }): Promise<any[]> {
    ...
    return plans.map(p => normalise(
      SearchEntityType.PDI, p.id, p.name,
  ...
  private async searchCompetencies(q: string, opts: { limit: number }): Promise<any[]> {
    const comps = await (this.prisma as any).competency.findMany({
      where: { OR: [{ name: iLike(q) }, { description: iLike(q) }] },
      select: { id: true, name: true, type: true, description: true },
      take:   opts.limit,
    }).catch(() => [] as any[]);

    return (comps as any[]).map((c: any) => normalise(
      SearchEntityType.COMPETENCY, c.id, c.name, c.type ?? '',
  ...
  private async searchScenarios(q: string, opts: { limit: number }): Promise<any[]> {
    const scenarios = await (this.prisma as any).avatarScenario.findMany({
      where:  { OR: [{ title: iLike(q) }, { description: iLike(q) }], active: true },
      select: { id: true, title: true, category: true, difficulty: true },
      take:   opts.limit,
    }).catch(() => [] as any[]);

    return (scenarios as any[]).map((s: any) => normalise(
      SearchEntityType.SCENARIO, s.id, s.title,
  ...
  async searchByType(type: SearchEntityType, q: string, userId: number, dto: TypedSearchDto) {
    ...
    let results: any[] = [];
    switch (type) {
      case SearchEntityType.USER:       results = await this.searchUsers(q, opts); break;
      case SearchEntityType.COURSE:     results = await this.searchCourses(q, opts); break;
      case SearchEntityType.DOCUMENT:   results = await this.searchDocuments(q, { limit }); break;
      case SearchEntityType.CONTENT:    results = await this.searchContent(q, { limit }); break;
      case SearchEntityType.PDI:        results = await this.searchPdis(q, { limit, userId }); break;
      case SearchEntityType.COMPETENCY: results = await this.searchCompetencies(q, { limit }); break;
      case SearchEntityType.SCENARIO:   results = await this.searchScenarios(q, { limit }); break;
      default:                          results = [];
    }
  ...
  async autocomplete(q: string, userId: number, limit = 5) {
    ...
    const recentHistory = await safeM(this.prisma, 'searchHistory').findMany({
  ...
  async getSuggestions(userId: number) {
    const user = await this.prisma.user.findUnique({
      where:  { id: userId },
      select: { departmentId: true, userCompetencies: { select: { competencyId: true, currentLevel: true } } },
    });
    ...
    const trending = await safeM(this.prisma, 'searchHistory').groupBy({
      by:      ['query'],
      _count:  { id: true },
    ...
    return {
      recommendedCourses: suggestedCourses.map(c => normalise(SearchEntityType.COURSE, c.id, c.title, c.category ?? '', { thumbnailUrl: c.thumbnailUrl }, `/courses/${c.id}`)),
      popularContent:     popularContent.map(c => normalise(SearchEntityType.CONTENT, c.id, c.title, c.type ?? '', {}, `/content/${c.id}`)),
  ...
  async getHistory(userId: number, limit = 20) {
    const history = await safeM(this.prisma, 'searchHistory').findMany({
      where:   { userId },
      orderBy: { createdAt: 'desc' },
      take:    limit,
    }).catch(() => [] as any[]);
  ...
  async clearHistory(userId: number) {
    await safeM(this.prisma, 'searchHistory').deleteMany({ where: { userId } }).catch(() => null);
  ...
  async getAnalytics() {
    const [
      totalSearches, uniqueUsers,
      topTerms, zeroResults,
    ] = await Promise.all([
      safeM(this.prisma, 'searchHistory').count({}).catch(() => 0),
      safeM(this.prisma, 'searchHistory').groupBy({ by: ['userId'], _count: { id: true } }).then((r: any[]) => r.length).catch(() => 0),
      ...
      safeM(this.prisma, 'searchHistory').count({ where: { resultsCount: 0 } }).catch(() => 0),
    ]);
  ...
  private async trackSearch(userId: number, query: string, searchType: string, resultsCount = 0) {
    await safeM(this.prisma, 'searchHistory').create({
