import {
  ...
  IsBoolean, IsDateString, Min, Max, MaxLength,
...
export class CreateTemplateTaskDto {
  ...
  @ApiProperty({ enum: TaskCategory }) @IsEnum(TaskCategory)
  category!: TaskCategory;

  @ApiProperty({ enum: TaskType }) @IsEnum(TaskType)
  type!: TaskType;

  @ApiProperty({ enum: TaskPhase }) @IsEnum(TaskPhase)
  phase!: TaskPhase;

  @ApiProperty({ enum: ResponsibleRole }) @IsEnum(ResponsibleRole)
  ...
  @ApiProperty({ description: 'Ordem na lista' }) @IsInt() @Min(0)
  seq!: number;
}
...
export class CreateOnboardingPlanDto {
  @ApiProperty({ description: 'ID do colaborador' }) @IsInt()
  userId!: number;

  @ApiProperty({ description: 'ID do template' }) @IsInt()
  templateId!: number;
...
export class CompleteTaskDto {
  @ApiProperty({ description: 'ID da tarefa da instância' }) @IsInt()
...
export class SkipTaskDto {
  ...
  @ApiProperty({ description: 'Motivo de saltar a tarefa' }) @IsString()
  reason!: string;
}
...
export class ValidateDocumentDto {
  ...
  @ApiProperty({ enum: DocumentStatus }) @IsEnum(DocumentStatus)
...
export class SubmitOnboardingSurveyDto {
  ...
  @ApiProperty({ enum: SurveyMilestone }) @IsEnum(SurveyMilestone)
  milestone!: SurveyMilestone;

  @ApiProperty({ description: 'Nota 1-5' }) @IsInt() @Min(1) @Max(5)
  score!: number;

  @ApiPropertyOptional({ description: 'eNPS (-10 a 10)' })
  @IsOptional() @IsInt() @Min(-10) @Max(10)
  enps?: number;
...
export class OnboardingFilterDto {
  @ApiPropertyOptional({ enum: OnboardingStatus }) @IsOptional() @IsEnum(OnboardingStatus)
