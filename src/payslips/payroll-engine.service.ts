...
export class PayrollEngineService {
  ...
  async calculate(ctx: PayrollContext, period: string): Promise<PayrollResult> {
    ...
    lines.push({
      ...
      name: 'Salário Base',
      type: 'EARNING',
      value: +workedBaseSalary.toFixed(2),
      isTaxable: true,
      calcType: 'FIXED',
    ...
    if (absenceDays > 0) {
      lines.push({
        ...
        name: `Desconto por Faltas (${absenceDays}d)`,
        type: 'DEDUCTION',
        value: +(absenceDays * dailyRate).toFixed(2),
        isTaxable: false,
        calcType: 'FIXED',
        isEmployerCost: false,
      });
    ...
    const food =
      ctx.foodAllowance ?? compensation?.foodAllowance ?? config.defaultFoodAllowance ?? 0;
    if (food > 0) {
      lines.push({
        code: 'ALLOWANCE_FOOD',
    ...
    const transport =
      ctx.transportAllowance ??
      compensation?.transportAllowance ??
      config.defaultTransportAllowance ??
      0;
    if (transport > 0) {
      lines.push({
    ...
    if (ctx.overtimeHours && ctx.overtimeHours > 0) {
      ...
      lines.push({
        code: 'OVERTIME',
        name: `Horas Extras (${ctx.overtimeHours}h)`,
        type: 'EARNING',
        value: +overtimeValue.toFixed(2),
        isTaxable: true,
        calcType: 'FORMULA',
        isEmployerCost: false,
      });
    ...
    if (ctx.bonusAmount && ctx.bonusAmount > 0) {
      lines.push({
        ...
        name: 'Bónus',
        type: 'EARNING',
        value: +ctx.bonusAmount.toFixed(2),
        isTaxable: true,
        calcType: 'FIXED',
        isEmployerCost: false,
      });
    ...
    lines.push({
      code: 'INSS_EMPLOYEE',
      name: `INSS Colaborador (${(ssRate * 100).toFixed(0)}%)`,
      type: 'DEDUCTION',
      value: ssEmployee,
      isTaxable: false,
      calcType: 'PERCENT',
    ...
    if (healthRate > 0) {
      ...
      lines.push({
        ...
        name: `Seguro de Saúde (${(healthRate * 100).toFixed(0)}%)`,
        type: 'DEDUCTION',
        value: healthVal,
        isTaxable: false,
        calcType: 'PERCENT',
    ...
    if (ctx.advanceDeduction && ctx.advanceDeduction > 0) {
      lines.push({
        ...
        name: 'Adiantamento',
        type: 'DEDUCTION',
        value: +ctx.advanceDeduction.toFixed(2),
        isTaxable: false,
        calcType: 'FIXED',
        isEmployerCost: false,
      });
  ...
  calculateIRT(
    base: number,
    brackets: Array<{ min: number; max?: number | null; rate: number; deduction?: number | null }>,
  ): { irt: number; bracketLabel: string } {
