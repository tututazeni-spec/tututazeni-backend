...
import {
  CreateLiveClassDto, UpdateLiveClassDto,
  LiveChatMessageDto, PostClassResponseDto, LiveClassFilterDto,
...
export class LiveClassesController {
  ...
  findAll(@Query() filters: LiveClassFilterDto) { return this.svc.findAll(filters); }
  ...
  attendanceReport(@Param('id', ParseIntPipe) id: number) { return this.svc.getAttendanceReport(id); }
 
  @Post()
  @Roles('ADMIN', 'RH')
  @ApiOperation({ summary: 'Criar aula ao vivo' })
  create(@Body() dto: CreateLiveClassDto) { return this.svc.create(dto); }
  ...
  createPostEval(@Param('id', ParseIntPipe) id: number) { return this.svc.createPostEvaluation(id); }
