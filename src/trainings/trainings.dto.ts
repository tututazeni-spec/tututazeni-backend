...
export class CreateTrainingDto {
  ...
  @ApiProperty({ enum: TrainingType }) @IsEnum(TrainingType)
  type!: TrainingType;

  @ApiProperty({ enum: TrainingLevel }) @IsEnum(TrainingLevel)
  level!: TrainingLevel;

  @ApiPropertyOptional({ enum: TrainingStatus, default: TrainingStatus.DRAFT })
  @IsOptional() @IsEnum(TrainingStatus)
...
export class CreateTrainingSessionDto {
  ...
  @ApiProperty({ description: 'Data e hora da sessão' }) @IsDateString()
  sessionDate!: string;
  ...
  @ApiProperty({ description: 'Duração em minutos' }) @IsInt() @Min(15)
  durationMinutes!: number;

  @ApiProperty({ enum: SessionModality }) @IsEnum(SessionModality)
...
export class RegisterParticipantDto {
  ...
  @ApiPropertyOptional({ description: 'Registar mesmo que tenha vagas esgotadas? (vai para lista de espera)' })
  @IsOptional() @IsBoolean()
  allowWaitlist?: boolean;
}

export class UpdateParticipantStatusDto {
  @ApiProperty({ enum: ParticipantStatus }) @IsEnum(ParticipantStatus)
...
export class RateTrainingDto {
  ...
  @ApiProperty({ description: 'Avaliação 1-5' }) @IsInt() @Min(1) @Max(5)
  rating!: number;
...
export class TrainingFilterDto {
  ...
  @ApiPropertyOptional({ enum: TrainingType }) @IsOptional() @IsEnum(TrainingType)
  type?: TrainingType;

  @ApiPropertyOptional({ enum: TrainingLevel }) @IsOptional() @IsEnum(TrainingLevel)
  level?: TrainingLevel;

  @ApiPropertyOptional({ enum: TrainingStatus }) @IsOptional() @IsEnum(TrainingStatus)
  status?: TrainingStatus;
