...
describe('AttendanceService (progress)', () => {
  ...
  describe('reviewLeave', () => {
    const pendingLeave = {
      id: 1, userId: 5, status: 'PENDING', leaveType: 'ANNUAL',
      startDate: new Date('2026-08-01'), endDate: new Date('2026-08-05'), workDays: 5,
    };
    ...
    it('deve lançar NotFoundException se pedido não encontrado', async () => {
      ...
      await expect(service.reviewLeave(99, { status: 'APPROVED' } as any, 1))
        .rejects.toThrow(NotFoundException);
    });

    it('deve lançar BadRequestException se pedido já processado', async () => {
      mockPrismaBase.leaveRequest.findUnique.mockResolvedValue({ ...pendingLeave, status: 'APPROVED' });
      await expect(service.reviewLeave(1, { status: 'APPROVED' } as any, 1))
    ...
    it('deve aprovar pedido e criar registos de presença', async () => {
      ...
      const result = await service.reviewLeave(1, { status: 'APPROVED', reviewNotes: 'OK' } as any, 9);

      expect(mockPrismaBase.leaveRequest.update).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: 1 }, data: expect.objectContaining({ status: 'APPROVED' }) }),
      );
  ...
  describe('assignSchedule', () => {
    it('deve atribuir horário a utilizador', async () => {
      ...
      const result = await service.assignSchedule({
        userId: 1, scheduleId: 2, effectiveFrom: '2026-09-01',
      } as any);
      expect(result).toBeDefined();
  ...
  describe('createOvertime', () => {
    it('deve criar pedido de horas extra', async () => {
      ...
      const result = await service.createOvertime(3, {
        date: '2026-08-10', overtimeMinutes: 120, reason: 'Projecto urgente',
      } as any);
      expect(result).toBeDefined();
  ...
  describe('createJustification', () => {
    const baseRecord = {
      id: 1, userId: 3, date: new Date(), // today
      status: 'ABSENT', context: 'WORK',
    };
    ...
    it('deve criar justificativa de presença', async () => {
      ...
      const result = await service.createJustification(3, {
        attendanceId: 1, reason: 'Consulta médica', leaveType: 'SICK',
      } as any);
  ...
  describe('reviewJustification', () => {
    const justification = {
      id: 1, attendanceId: 5, userId: 3, status: 'PENDING',
    ...
    it('deve aprovar justificativa e actualizar presença para JUSTIFIED', async () => {
      ...
      mockPrismaBase.attendanceJustification.update.mockResolvedValue({ ...justification, status: 'APPROVED' });
      mockAR.update.mockResolvedValue({});

      await service.reviewJustification(1, { status: 'APPROVED', reviewNotes: 'OK' } as any, 9);

      expect(mockAR.update).toHaveBeenCalledWith(
    ...
    it('deve rejeitar justificativa sem actualizar presença', async () => {
      mockPrismaBase.attendanceJustification.findUnique.mockResolvedValue(justification);
      mockPrismaBase.attendanceJustification.update.mockResolvedValue({ ...justification, status: 'REJECTED' });
  ...
  describe('validateQrToken', () => {
    it('deve lançar BadRequestException se token inválido', async () => {
      await expect(service.validateQrToken('token-invalido', 1))
        .rejects.toThrow(BadRequestException);
    });
    ...
    it('deve lançar BadRequestException se token expirado', async () => {
      ...
      await expect(service.validateQrToken(generated.token, 5))
        .rejects.toThrow(BadRequestException);
    });
  });
});
