import {
  ...
  IsArray, IsNumber, IsDateString, Min, Max, MaxLength, ValidateNested,
...
export class CreateLeadershipProgramDto {
  ...
  @IsOptional() @IsEnum(ProgramStatus)
  ...
  @ApiPropertyOptional({ description: 'Data de início' }) @IsOptional() @IsDateString()
  startDate?: string;

  @ApiPropertyOptional({ description: 'Data de fim' }) @IsOptional() @IsDateString()
  endDate?: string;
}
...
export class UpdateParticipantProgressDto {
  @ApiProperty({ description: 'Progresso 0-100' }) @IsInt() @Min(0) @Max(100)
  progress!: number;

  @ApiPropertyOptional({ enum: ParticipantStatus })
  @IsOptional() @IsEnum(ParticipantStatus)
...
export class CreateOneOnOneDto {
  @ApiProperty({ description: 'ID do liderado' }) @IsInt()
  subordinateId!: number;

  @ApiProperty({ description: 'Data e hora agendada' }) @IsDateString()
  scheduledAt!: string;
...
export class CompleteOneOnOneDto {
  ...
  @ApiProperty({ description: 'Ata da reunião' }) @IsString()
  minutes!: string;
...
export class Leadership360ResponseDto {
  @ApiProperty({ enum: LeadershipCompetency }) @IsEnum(LeadershipCompetency)
  competency!: LeadershipCompetency;

  @ApiProperty({ description: 'Score 1-5' }) @IsInt() @Min(1) @Max(5)
  score!: number;
}

export class Submit360FeedbackDto {
  @ApiProperty({ description: 'ID do líder avaliado' }) @IsInt()
  leaderId!: number;
  ...
  @IsArray() @ValidateNested({ each: true }) @Type(() => Leadership360ResponseDto)
...
export class SubmitPulseDto {
  ...
  @ApiProperty({ description: 'Score geral (1-5)' }) @IsInt() @Min(1) @Max(5)
  overallScore!: number;
...
export class CreateMentoringDto {
  @ApiProperty({ description: 'ID do mentor' }) @IsInt()
  mentorId!: number;

  @ApiProperty({ description: 'ID do mentorado' }) @IsInt()
  menteeId!: number;
...
export class LogMentoringSessionDto {
  ...
  @ApiProperty({ description: 'Data da sessão' }) @IsDateString()
  sessionDate!: string;
...
export class SendKudosDto {
  @ApiProperty({ description: 'ID do destinatário' }) @IsInt()
  receiverId!: number;
...
export class LeadershipFilterDto {
  @ApiPropertyOptional({ enum: ProgramLevel }) @IsOptional() @IsEnum(ProgramLevel)
  level?: ProgramLevel;

  @ApiPropertyOptional({ enum: ProgramStatus }) @IsOptional() @IsEnum(ProgramStatus)
  status?: ProgramStatus;
