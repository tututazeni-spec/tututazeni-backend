...
import {
  ...
  CreateOneOnOneDto, UpdateOneOnOneDto, CompleteOneOnOneDto,
  Submit360FeedbackDto, SubmitPulseDto,
  CreateMentoringDto, LogMentoringSessionDto,
  UpsertTeamHealthDto, SendKudosDto, LeadershipFilterDto,
...
export class LeadershipService {
  ...
  async remove(id: number) {
    ...
    if (p._count.participants > 0) {
      throw new BadRequestException('Programa com participantes não pode ser eliminado. Archive-o primeiro.');
    }
  ...
  async updateProgress(userId: number, programId: number, dto: UpdateParticipantProgressDto) {
    ...
    const updated = await this.prisma.leadershipParticipant.update({
      ...
      data: {
        ...
        status:      isCompleting ? 'COMPLETED' : (dto.status ?? (dto.progress > 0 ? 'IN_PROGRESS' : participant.status)),
  ...
  async getProgramStats(programId: number) {
    ...
    return {
      programId, total, completed, inProgress,
...
const pendingLeaves = 0; // TODO: implementar LeaveRequest
  ...
  async getTeamHealth(managerId: number) {
    ...
    const teamIds = (await this.prisma.user.findMany({
      where: { managerId, active: true }, select: { id: true },
    })).map(u => u.id);
    ...
    const evaluationsOnTimePct = totalReviews > 0
      ? Math.round((completedReviews / totalReviews) * 100)
      : 0;
    const pdisCompletedPct = totalPdis > 0
  ...
  async getOneOnOnes(managerId: number, subordinateId?: number) {
    ...
    return this.prisma.oneOnOne.findMany({
      where,
      include: {
        subordinate: { select: { id: true, fullName: true, avatarUrl: true, position: { select: { name: true } } } },
      },
  ...
  async get360Summary(leaderId: number) {
    ...
    const byCompetency = Object.entries(compMap).map(([competency, scores]) => ({
      ...
      count:        scores.length,
      insight:      scores.filter(s => s <= 2).length / scores.length >= 0.5
        ? `⚠ ${Math.round((scores.filter(s => s <= 2).length / scores.length) * 100)}% indicam lacuna`
        : null,
    }));

    const allScores  = Object.values(compMap).flat();
    const avgScore   = Math.round((allScores.reduce((s, v) => s + v, 0) / allScores.length) * 10) / 10;
  ...
  async createMentoring(dto: CreateMentoringDto) {
    ...
    if (existing) throw new ConflictException('Relação de mentoring já existe entre estes utilizadores');
  ...
  async getMyMentoring(userId: number) {
    const [asMentor, asMentee] = await Promise.all([
      this.prisma.mentoring.findMany({
        ...
        include: {
          mentee:   { select: { id: true, fullName: true, avatarUrl: true, position: { select: { name: true } } } },
  ...
  async recalcLeadershipScore(leaderId: number) {
    ...
    const score = Math.round(
      teamHealthScore  * 0.30 * 10 +  // max 300
      developmentScore * 0.25 * 10 +  // max 250
      feedback360Score * 0.20 * 10 +  // max 200
      punctualityScore * 0.15 * 10 +  // max 150
      Math.min(100, programsCompleted * 20) * 0.10 * 10  // max 100
    );
