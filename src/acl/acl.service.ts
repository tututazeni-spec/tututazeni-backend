...
import {
  ...
  AclAuditFilterDto, PermissionAction, PermissionSubject,
} from './acl.dto';
...
function cacheGet(userId: number) {
  ...
  if (Date.now() - entry.cachedAt > CACHE_TTL) { permCache.delete(userId); return null; }
  return entry;
}
...
const ROLE_DEFAULTS: Record<string, string[]> = {
  ADMIN:      ['*'],
  RH:         ['dashboard:view', 'dashboard:export', 'reports:view', 'reports:create', 'reports:export',
                'users:view', 'users:create', 'users:update', 'lms:view', 'lms:create', 'lms:update',
                'performance:view', 'performance:view_all', 'performance:create', 'performance:approve',
                'engagement:view', 'engagement:create', 'talent:view', 'talent:create', 'talent:approve',
                'evaluation:view', 'evaluation:create', 'content:view', 'content:create',
                'payroll:view', 'payroll:export', 'sensitive:view', 'roi:view', 'history:view', 'history:export'],
  LIDER:      ['dashboard:view', 'users:view', 'lms:view', 'performance:view', 'performance:create',
                'performance:approve', 'engagement:view', 'talent:view', 'talent:create',
                'evaluation:view', 'evaluation:create', 'content:view', 'history:view', 'avatar:view'],
  COLABORADOR:['dashboard:view', 'lms:view', 'content:view', 'avatar:view', 'engagement:view',
                'performance:view', 'history:view'],
  INSTRUCTOR: ['lms:view', 'lms:create', 'lms:update', 'content:view', 'content:create', 'avatar:view', 'avatar:create'],
  AUDITOR:    ['dashboard:view', 'reports:view', 'reports:export', 'users:view', 'lms:view',
                'performance:view', 'history:view', 'history:export', 'sensitive:view'],
  DIRECTOR:   ['dashboard:view', 'dashboard:export', 'reports:view', 'reports:export',
                'users:view', 'performance:view', 'performance:view_all', 'talent:view',
                'roi:view', 'history:view'],
};
...
export class AclService {
  ...
  async getUserPermissions(userId: number) {
    ...
    if (cached) return { userId, roleCode: cached.roleCode, permissions: cached.permissions, cached: true };
    ...
    const effective = (roleCode === 'ADMIN' || permissions.includes('*'))
  ...
  async getPolicies() {
    return (this.prisma as any).accessPolicy?.findMany({ orderBy: { priority: 'desc' } }).catch(() => [] as any[]);
  }

  async createPolicy(dto: CreatePolicyDto, createdById: number) {
    return (this.prisma as any).accessPolicy?.create({
      data: {
        name:                  dto.name,
        description:           dto.description,
        subject:               dto.subject,
    ...
    }).catch(() => ({ message: 'Política registada (modelo accessPolicy ausente — execute migration)', ...dto }));
  }

  private async evaluatePolicies(
    userId: number, action: string, subject: string, context?: Record<string, any>,
  ): Promise<boolean> {
    // DENY = true means access denied
    const policies = await (this.prisma as any).accessPolicy?.findMany({
      where: {
        active: true,
        effect: 'DENY',
        ...(subject ? { subject } : {}),
  ...
  async getPermissionMatrix() {
    ...
    return { roles: roles.map(r => ({ id: r.id, name: r.name, code: r.code })), permissions, matrix };
  }
  ...
  async getStats() {
    ...
    const roleBreakdown = await this.prisma.user.groupBy({
      ...
      const roles = await this.prisma.role.findMany({ where: { id: { in: ids } }, select: { id: true, name: true, code: true } });
