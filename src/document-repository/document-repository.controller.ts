...
import {
  ...
  GrantPermissionDto, CreateShareLinkDto,
...
export class DocumentRepositoryController {
  ...
  getStats(@Query('department') department?: string) { return this.svc.getStats(department); }
 
  @Get('tags')
  @ApiOperation({ summary: 'Todas as tags com contagem (para cloud de tags)' })
  getTags() { return this.svc.getAllTags(); }
 
  @Get('expiring-soon')
  @Roles('ADMIN', 'RH')
  @ApiOperation({ summary: 'Documentos a expirar nos próximos N dias' })
  @ApiQuery({ name: 'days', required: false, type: Number })
  getExpiringSoon(@Query('days') days?: string) { return this.svc.getExpiringSoon(days ? +days : 30); }
  ...
  createCategory(@Body() dto: CreateDocCategoryDto) { return this.svc.createCategory(dto); }
  ...
  getAuditLog(@Param('id', ParseIntPipe) id: number) { return this.svc.getAuditLog(id); }
 
  @Get(':id/access-log')
  @Roles('ADMIN', 'RH')
  @ApiOperation({ summary: 'Histórico de downloads por utilizador' })
  getAccessLog(@Param('id', ParseIntPipe) id: number) { return this.svc.getAccessLog(id); }
  ...
  processExpired() { return this.svc.processExpiredDocuments(); }
}
