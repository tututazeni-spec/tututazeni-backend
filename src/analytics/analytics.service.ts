...
export class AnalyticsService {
  ...
  async getOrganizationOverview() {
    const [
      ...
      this.prisma.microLearning.count({ where: { active: true } }),
    ]);
  ...
  async getEngagementMetrics(filters: AnalyticsFilterDto) {
    ...
    const users = await this.prisma.user.findMany({
      where: userWhere, select: { id: true },
    });
    ...
    const [
      ...
      this.prisma.microLearningDispatch.count({
        where: { userId: { in: userIds }, viewed: true },
      }),
      this.prisma.knowledgeInteraction.count({
  ...
  async getTrainingROI() {
    ...
    const totalHoursInvested = courseAnalytics.reduce((acc, ca) => {
      return acc + (ca.course.workloadHours ?? 0) * ca.totalCompleted;
    }, 0);
