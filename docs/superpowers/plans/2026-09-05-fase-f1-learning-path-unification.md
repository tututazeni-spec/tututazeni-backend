...
> **ESTADO (2026-09-06) — FASE F1 CONCLUÍDA:**
> - **F1a — FEITO** (PR #260): a Task 2 foi substituída por **remoção** dos endpoints `/content-library/paths/*` (eram no-ops via `safeModel`, shape de `LearningPath` inexistente, sem cobertura de integração) — decisão do dono do produto: código morto, não vale rewire. `content-library` já não tem learning paths.
> - **F1b — NÃO SE FAZ** (PR #262, docs). Decisão do dono do produto (2026-09-06): **`LmsLearningPath` não se migra**. Não é uma 3ª cópia do conceito canónico — é uma feature distinta: `courseIds String[]` são **códigos opacos** (a `lms.integration-spec.ts` usa `"course-a"`/`"course-b"`, não FKs `Course`), progresso **auto-reportado** (`PUT /lms/paths/:id/progress` → append a `completedCourseIds[]`), chave humana `code @unique`, `isFeatured`. O canónico usa FKs `Course` reais, progresso derivado de `Enrollment.status`, milestones, assignments organizacionais, publish workflow. Fundir mudaria materialmente o comportamento do LMS ou poluiria o modelo canónico. `/lms/paths*` fica como feature própria. **Tasks 3–8 abaixo NÃO se executam.**
