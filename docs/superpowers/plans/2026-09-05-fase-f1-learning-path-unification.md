...
> - **F1b — POR FAZER**: Tasks 3–8 (migração `LmsLearningPath` → `LearningPath` com `legacyLmsId` + backfill + adaptador). **Fork por resolver:** `PUT /lms/paths/:id/progress` recebe um `completedCourseId` auto-reportado e append a `LmsPathEnrollment.completedCourseIds[]`; o canónico deriva progresso de `Enrollment.status === 'COMPLETED'` — **não há método canónico que aceite um sinal por-curso**. Decidir: (a) o adaptador cria/completa um `Enrollment` real para esse curso+user (muda comportamento — o user fica "inscrito" no curso), ou (b) manter estado de progresso LMS-específico (contraria a fusão), ou (c) outra via. A `lms.integration-spec.ts` cobre bem este endpoint — regressão a vigiar.
