...
| G | Sub-fase dedicada: PDI/Sucessão/1:1 no domínio Talento & Performance — **em curso** em 4 sub-fases: **G1 concluída** (`Competency` → `CompetenciesService`; `evaluation360` delega — ver item 7), **G2 concluída** (`SuccessionPlan` → `SuccessionService`; `career` delega — ver item 8), **G3 concluída** (PDI → `DevelopmentPlansService`; `talent-development` + `leader` delegam, fecha o buraco de auditoria de `activatePlan` — ver item 9), G4 (1:1 → `OneOnOneService`/`OneOnOneMeeting`) | 8 | Alto (maior densidade de duplicação, mais fluxos de estado) | Maior, deliberadamente deixado para depois de validar o padrão nas fases A–F. Ver `docs/superpowers/plans/2026-09-05-fase-g{1,2,3,4}-*.md` |
