...
| 5 | **Certificação/Badges** — **certificados resolvidos (Fase F2)**: `Certificate` é o modelo único. `IssuedCertificate` foi absorvido — as suas colunas ricas (`hashCode`, `title`, `recipientName`, revogação com autor/motivo/data, contadores, `issuedById`, `templateId`) passaram para `Certificate`; os dados foram migrados (`legacyIssuedCertId` + backfill idempotente); `CertificationService` e `dashboard-institutional` passaram a `Certificate`; `/certification/*` mantém a forma histórica via adaptador (`legacyType` guarda o `CertificateTemplateType` original — a tradução para `CertificateType` é lossy). `IssuedCertificate`/`CertificateTemplate` ficam no schema (deprecados; remoção = follow-up). **Badges continuam por unificar (F3)**: `Badge`/`BadgeAward` (avatar-training) vs `DigitalBadge`/`BadgeIssuance` (certification). | Era 4 modelos / 2 pares; agora `Certificate` único + 1 par de badges | Certificação unificada; badges ainda com dois sistemas |
...
| F | **F1 + F2 concluídas.** F1: F1a removeu os endpoints mortos de `content-library`; F1b decidiu **não** migrar `LmsLearningPath` (feature distinta — ver item 6). **F2**: `IssuedCertificate` absorvido por `Certificate` (`legacyType`/`legacyIssuedCertId` + backfill + adaptador); `dashboard-institutional` conta `Certificate`. Falta **F3** (badges: `DigitalBadge`/`BadgeIssuance` vs `Badge`/`BadgeAward`). | 6, 7 | Médio-alto (muitas dependências de frontend a confirmar) | Estruturalmente necessário antes de qualquer extracção do domínio Academia |
