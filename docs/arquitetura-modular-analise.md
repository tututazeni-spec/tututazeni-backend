...
> **Actualização (Fase J, 2026-09-07):** `automation.executeAction` deixou de escrever `Enrollment`/`DevelopmentPlan`/`UserPoints`/`BadgeAward` por Prisma directo — `ASSIGN_COURSE` delega em `EnrollmentsService.enroll` (recupera as guardas de matrícula duplicada, curso `PUBLISHED`, `CourseAnalytics++`, notificação `COURSE_ENROLLED`), `CREATE_PDI` em `DevelopmentPlansService.create` (o PDI entra no fluxo de aprovação — status `DRAFT` — e notifica), `AWARD_POINTS`/`AWARD_BADGE` no novo `GamificationService` (dono único de `awardPoints`/`awardBadge`; `CourseCompletionService` também passou a delegar). **Mudança de comportamento deliberada:** uma acção de automação com input inválido (matrícula duplicada, curso não publicado) passa a ser registada em `AutomationExecution` como `FAILED` em vez de criar dados inválidos ou rebentar 500. `scalability.triggerSync` delega o `IntegrationSyncLog` (RUNNING) em `ApiIntegrationService.recordSync`. **Follow-up:** o *merge* de `createIntegration`/`updateIntegration` entre `scalability` e `api-integration` fica por fazer — divergem em encriptação de credenciais, validação de tenant, `authType` e forma do DTO (`CreateIntegrationConfigDto` vs `CreateIntegrationDto`); e `scalability.events.ts` ainda faz `integrationSyncLog.update` directo no worker de sync.
...
> **Actualização (Fase J, 2026-09-07):** a entrega de webhooks (`api-integration`) e o email de convite de utilizador (`users.service.invite`) passaram para filas Bull dedicadas (`webhooks` + `WebhooksProcessor`, `email` + `EmailProcessor`) — retry/backoff geridos pelo Bull, fora do caminho síncrono do request. `POST /users/invite` responde mais depressa e cria o utilizador mesmo com o SMTP em baixo (antes, um SMTP indisponível impedia a criação). `testIntegration` mantém um `HEAD` síncrono de 5s (teste manual explícito, não entrega). **Ainda como follow-up:** exportação XLSX/PDF pesada e `automation.runAllActiveRules` em loop sequencial.
...
- `ApiKey`/`Webhook`/`WebhookDelivery` não existem no schema; `safeM()` degrada graciosamente (200 OK, zero persistência) — já documentado no `CLAUDE.md`, confirmado sem alteração. **(Fase J:** a entrega de webhooks passou para a fila `webhooks`, mas a *persistência* de `WebhookDelivery` continua a degradar via `safeM()` — migração desses três modelos fica como follow-up, tal como a substituição do Base64 por KMS/Key Vault nas credenciais de integração, que é um item de segurança separado.**)**
...
| J | ~~`automation`/`scalability` passam a chamar serviços de domínio em vez de Prisma directo; mover webhooks/emails para a fila Bull~~ — **concluída (2026-09-07)**: `automation.executeAction` delega em `EnrollmentsService`/`DevelopmentPlansService`/novo `GamificationService` (guardas de matrícula/PDI/pontos/badge recuperadas; input inválido → acção `FAILED`, não 500); `scalability.triggerSync` delega o `IntegrationSyncLog` em `ApiIntegrationService.recordSync`; webhooks (`WebhooksProcessor`) e email de convite (`EmailProcessor`) em filas Bull dedicadas. **Follow-ups:** merge de `createIntegration`/`updateIntegration` `scalability`↔`api-integration` (encriptação de credenciais + DTO), persistência de `WebhookDelivery`/`ApiKey`/`Webhook`, `runAllActiveRules` sequencial, export XLSX/PDF pesado. | 9, 12 | Médio | Ver `docs/superpowers/plans/2026-09-05-fase-j-*.md` + `notes/fase-j-map.md` |

**Roteiro A–J completo.** Restam apenas follow-ups pontuais (remoção física de modelos legados deprecados nas fases E/F/G, os follow-ups da linha J, e a escrita de `DashboardSnapshot` da fase H) — nenhum estrutural.
