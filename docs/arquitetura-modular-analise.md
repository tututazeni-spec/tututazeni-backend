...
| F | ~~Unificar Learning Path~~ **F1 concluída**: F1a removeu os endpoints mortos de `content-library`; F1b decidiu **não** migrar `LmsLearningPath` (feature distinta — ver item 6). Falta Certificação/Badges (2→1 cada) [F2/F3]. | 6, 7 | Médio-alto (muitas dependências de frontend a confirmar) | Estruturalmente necessário antes de qualquer extracção do domínio Academia |
